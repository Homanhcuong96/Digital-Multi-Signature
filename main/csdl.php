...
function createGroup($file, $group_name) {
	$cmd = "python python_script/generate_hash_digest.py $file";
	$hash_value = shell_exec($cmd);
	$conn=ketnoi();

    $sql="INSERT  INTO  groups (hash_value, group_name, public_key, public_value) VALUES  ('$hash_value','$group_name',-1, -1)";

	$result = mysqli_query($conn, $sql);
	if ($result != FALSE) {
		echo "<script>alert('Tạo nhóm thành công')</script>";
	}
    mysqli_close($conn);
}
function getGroupWithName($group_name) {
	$conn = ketnoi();
	$sql = "select group_id from groups where groups.group_name='$group_name'";
...
function getPrivateKey($private_key) {
	$conn = ketnoi();
	$sql = "select private_key from user_keys where user_keys.private_key='$private_key'";
...
function userInitKey($user_name, $group_name, $private_key, $private_value) {
	if ($user_name == '' || $group_name == '' || $private_key == '' || $private_value == '') {
		echo "<script>alert('Không có giá trị nhập vào')</script>";
		return;
	}
	$conn=ketnoi();

	$group_id = getGroupWithName($group_name);
	if (count($group_id) == 0) {
		echo "<script>alert('Không tìm thấy nhóm')</script>";
	} else {
		$group_id = $group_id[0];
		$sql="INSERT INTO user_keys (user_name, group_id, private_key, private_value) VALUES ('$user_name', '$group_id', '$private_key', '$private_value')";
		$result = mysqli_query($conn, $sql);
...
function ketqua($group_name) {
	...
	if (count($group_id) == 0) {
		...
		$group_id = $group_id[0];
		$sql = "SELECT part_1,part_2 from group_signature WHERE group_signature.group_id='$group_id'";
		$result = mysqli_query($conn, $sql);
		$value = mysqli_fetch_array($result);
		if (count($value) == 0) {
			$cmd = "python python_script/generate_signature.py $group_id";
			shell_exec($cmd);
			$sql = "SELECT part_1,part_2 from group_signature WHERE group_signature.group_id='$group_id'";
			$result = mysqli_query($conn, $sql);
			$value = mysqli_fetch_array($result);
		...
		$sql = "SELECT public_key,public_value from groups WHERE groups.group_id='$group_id'";
		$result = mysqli_query($conn, $sql);
		$value = mysqli_fetch_array($result);
		$public_key = $value[0];
		$public_value = $value[1];

		$sql = "SELECT q from group_keys WHERE group_keys.group_id='$group_id'";
		$result = mysqli_query($conn, $sql);
		$value = mysqli_fetch_array($result);
		$q = $value[0];
		//return [$public_key, $public_value, $part_1, $part_2, $q];
		return [$part_1, $part_2, $public_key, $public_value, $q];
	}
	mysqli_close($conn);
}

function get_hash_digest($file) {
	$cmd = "python python_script/generate_hash_digest.py $file";
	$hash_value =shell_exec($cmd);
	return $hash_value;
}

function verify_integrity($part_1, $part_2, $public_key, $public_value, $q, $hash_value) {
	$cmd = "python python_script/verify_signature.py $part_1 $part_2 $public_key $public_value $q $hash_value";
	$output = shell_exec($cmd);
	if ($output) {
		echo "<script>alert('Chữ ký hợp lệ')</script>";
	}
}
?>